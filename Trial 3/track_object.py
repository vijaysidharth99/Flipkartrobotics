...
class TrackObject:
    def __init__(self, color):
        ...
        if color == 'pink':
            ...
            self.hsv_ub = np.array([100, 255, 255])
        ...
        for cnt in contours:
            ...
            if area > 1000:
                ...
                if len(approx) == 4:  # if four end points are detected
                    ...
                    if self.count%10 == 0:
                        print('saving')
                        cv2.imwrite("image4%04i.jpg" %self.count, cropped_img)
                    cv2.waitKey(1)
                    circle2 = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 8.5, 100, param1=50, param2=2, minRadius=5,
                                               maxRadius=10)
                    # ensure at least some circles were found
                    if circle2 is not None:
                        # convert the (x, y) coordinates and radius of the circles to integers
                        circle2 = np.round(circle2[0, :]).astype("int")
                        circle2_x = circle2[0, 0] + x
                        circle2_y = circle2[0, 1] + y
                        cv2.circle(img_og, (circle2_x, circle2_y), circle2[0, 2], (0, 255, 0), 4)
                        cv2.circle(img_og, (circle1_x, circle1_y), 5, [255, 255, 0], 2)
                        A = [[math.cos(2.44346), -math.sin(2.44346)],
                             [math.sin(2.44346), math.cos(2.44346)]]
                        B = [[circle2_x - circle1_x],
                             [circle2_y - circle1_y]]
                        end_point = np.dot(A, B)
                        end_point_x = int(end_point[0] + circle1_x)
                        end_point_y = int(end_point[1] + circle1_y)
                        print('endpoint: ', (end_point_x, end_point_y))
                        cv2.arrowedLine(img_og, (circle1_x, circle1_y), (end_point_x, end_point_y), [255, 255, 0], 5)
